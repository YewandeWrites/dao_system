...
shared actor class DAO(init : Types.BasicDaoStableStorage) = Self {
    ...
    public query func intitalize_voter_data() : async Result.Result<Text, Text> {
        return #ok "initialized voter data";
    };

    public shared ({ caller }) func move_vote_power(transfer : Types.TransferArgs) : async Types.Result<(), Text> {
    ...
    public query ({ caller }) func voter_token_balance() : async Types.Tokens {
        Option.get(account_get(caller), Types.zeroToken);
    };

    public query func list_voters() : async [Types.Account] {
        Iter.toArray(
            Iter.map(
                Trie.iter(accounts),
